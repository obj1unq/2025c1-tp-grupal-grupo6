...
class Tipo {
  ...
  method aplicarEfecto(alumno) {
    alumno.position(alumno.position().down(1))
  }
...
object amarillo inherits Tipo {
  ...
  override method aplicarEfecto(alumno) {
    alumno.position(alumno.position().up(2))
  }
}
...
object policia inherits Tipo {
  ...
  override method aplicarEfecto(alumno) {
    alumno.position(game.at(self.ramdom(), 4))
  }
}
...
object verde inherits Tipo {
  ...
  override method aplicarEfecto(alumno) {
    super(alumno)
    if ((alumno.position().x() + self.ramdom()) >= game.width())
      self.reasignarX(alumno, 14)
    else self.reasignarX(alumno, self.ramdom())
  }
  
  method reasignarX(alumno, posicion) {
    alumno.position(game.at(posicion, alumno.position().y()))
  }
}